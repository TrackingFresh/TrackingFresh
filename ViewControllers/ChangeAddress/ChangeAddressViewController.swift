...
class ChangeAddressViewController: UIViewController,UITableViewDelegate,UITableViewDataSource
{
    
    // MARK: - IBOutlets & Objects
    
    @IBOutlet weak var ChnageAddresTableView: UITableView!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        ChnageAddresTableView.delegate = self
        ChnageAddresTableView.dataSource = self
        ChnageAddresTableView.tableFooterView = UIView()
         //self.title = "Change Address"
        self.customNavigationItem(stringText: "CHANGE ADDRESS", showbackButon: false)
 ChnageAddresTableView.register(UINib(nibName: "AddressTableViewCell", bundle: nil), forCellReuseIdentifier: "AddressTableViewCell")
            ...
            WebserviceHelper.sharedInstance.callGetDataWithMethod(strMethodName:"address", requestDict: [:], isHud: false, hudView: self.view, value: headerDict, successBlock:
                {  response, responseStr in print(response)
                    ...
                    if let httpStatus = response as? HTTPURLResponse, httpStatus.statusCode == 200{
                        ...
                        self.ChnageAddresTableView.reloadData()
                    }
                    else{
...
//        self.navigationController?.pushViewController(updateAddressViewController, animated: true)
        updateAddressViewController.adddressDict = AddressDict as! [String : Any]
